...
struct VersionChainRecord {
  VersionChainRecord(version_t v, weight_t w, EdgeOperation operation)
      : v(v), w(w), operation(operation) {};
  version_t v;
  weight_t w;
...
forward_list<VersionChainRecord>::iterator get_version_from_chain(
    forward_list<VersionChainRecord> &chain, version_t version) {
  count_x++;
  count_y++;
  auto i = chain.begin();
  while (i->v > version) {
    count_y++;
    i++;
    assert(i != chain.end());  // This should not happen as the last entry in a
                               // chain has v == FIRST_VERSION.
  }
  return i;
}

EdgeVersionRecord::EdgeVersionRecord(dst_t e, version_t *v, char *w,
                                     bool has_weight, size_t weight_size)
...
void EdgeVersionRecord::gc(version_t min_version,
  ...
  if (state == MULTIPLE_VERSIONS) {
    ...
    auto current = chain->begin();  // Will point to the version read by the
                                    // current active version
    auto before = chain->begin();   // Will point to the version read by the
                                    // active version before current.
    ...
    // We do not collect before to not complicate the process of building the
    // list of sorted active transactions.
    ...
    // Current is now the oldest version read by any transaction. Mark it as
    // FIRST_VERSION.
...
void EdgeVersionRecord::write(version_t version, EdgeOperation kind,
                              char *weight) {
  assert(has_weight);
...
void EdgeVersionRecord::create_version_chain() {
  ...
  switch (state) {
    ...
    case TWO_VERSIONS: {
      ...
      chain->push_front(
          VersionChainRecord(timestamp(*v), *w, second_operation));
      *v = (version_t)chain;
      *v |= MORE_VERSION_MASK;
      state = MULTIPLE_VERSIONS;
      break;
...
void EdgeVersionRecord::assert_can_write(version_t version,
                                         EdgeOperation operation) {
#ifdef DEBUG
