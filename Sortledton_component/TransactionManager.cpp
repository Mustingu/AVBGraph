...
SnapshotTransaction TransactionManager::getSnapshotTransaction(
    STL_VersionedTopologyInterface *ti, bool write_only) {
  return SnapshotTransaction(this, write_only, ti);
}

void TransactionManager::getSnapshotTransaction(
    STL_VersionedTopologyInterface *ti, bool write_only,
...
void TransactionManager::transactionCompleted(
    const SLT_Transaction &transaction) {
  if (transaction.get_version() != active_snapshots[thread_id] &&
      transaction.get_commit_version() != active_snapshots[thread_id]) {
    throw IllegalOperation(
        "Thread tried to complete transaction, it did not open.");
  }
  active_snapshots[thread_id] = NO_TRANSACTION;
}

TransactionManager::TransactionManager(uint max_threads, bool on)
    : max_threads(max_threads) {
  reset_max_threads(max_threads);
  stopped.store(false);
  if (on)
    min_version_updater = thread(&TransactionManager::run_min_version_updater,
                                 this, MIN_VERSION_UPDATER_INTERVAL);
}
...
version_t TransactionManager::getMinActiveVersion() { return min_version; }

void TransactionManager::update_min_version() {
  min_version = min(version.load(), *min_element(active_snapshots.begin(),
...
void TransactionManager::deregister_thread(size_t id) {
  ...
  if (!thread_id_in_use[id]) {
    throw IllegalOperation(
        "Trying to deregister a thread that has not been registered");
  }
  if (active_snapshots[id] != NO_TRANSACTION) {
    throw IllegalOperation(
        "Trying to deregister a thread with an active transaction");
  }
...
version_t TransactionManager::draw_timestamp(bool commit_timestamp) {
  if (!commit_timestamp && active_snapshots[thread_id] != NO_TRANSACTION) {
    throw IllegalOperation(
        "Cannot have more than one transaction open per thread.");
  }
  ...
  if (commit_timestamp && active_snapshots[thread_id] == NO_TRANSACTION) {
    ...
    return version.fetch_add(
        1);  // Return a new commit version for a SnapshotTransaction which has
             // a read version already.
  } else {
    active_snapshots[thread_id] = version.fetch_add(1);
    return active_snapshots[thread_id];
  }
