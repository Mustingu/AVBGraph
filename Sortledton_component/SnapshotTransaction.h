...
class RollbackAction {
 public:
  enum ROLLBACK_ACTION { INSERT_VERTEX };
 ...
 private:
  RollbackAction(ROLLBACK_ACTION type, vertex_id_t v, edge_t e)
      : type(type), vertex(v), edge(e) {};
};

class TransactionManager;

class SnapshotTransaction : public SLT_Transaction {
 public:
  SnapshotTransaction(TransactionManager* tm, bool write_only,
                      STL_VersionedTopologyInterface* ds);
  ~SnapshotTransaction();
  ...
  bool insert_edge(edge_t edge, char* properties,
                   size_t property_size) override;
  bool insert_or_update_edge(edge_t edge, char* properties,
                             size_t property_size) override;
   ...
   *
   * Passing any other version than the one of this transaction is undefined
   * behaviour.
   *
   * Using the pointer after calling execute or
   * TransactionManager.transactionCompleted is undefined behaviour.
   */
  STL_VersionedTopologyInterface* raw_ds();

  void intersect_neighbourhood_p(vertex_id_t a, vertex_id_t b,
                                 vector<dst_t>& out) override;
 ...
 private:
  // TODO check how much performanc it cost to make this class thread safe by a
  // mutex on each writing function. if this is to expensive reintroduce a
  // thread safe readonly transaction and note about thread safety in the
  // documentation.
  ...
  /**
   * Used for communication between aquire_ and release_locks. This is the last
   * lock aquire_locks locked. release_lock will only call unlock for locks up
   * till then.
   */
  ...
  /**
   * Used to store information on the number of vertices in the data structure
   * at the time of transaction start.
   *
