...
SnapshotTransaction::SnapshotTransaction(TransactionManager *tm,
                                         bool write_only,
  ...
  try {
    ...
    for (auto v : vertices_to_delete) {
      if (vertex_does_not_exists_semantic_activated &&
          !ds->has_vertex_version(v, commit_version)) {
        continue;
      }
    ...
    for (auto e : edges_to_delete) {
      ...
      if (edge_does_not_exists_semantic_activated &&
          !ds->has_edge_version_p(p_edge, commit_version)) {
        continue;
      }
    ...
    for (auto [e, properties, properties_size] : edges_to_insert) {
      ...
      ds->insert_edge_version(p_edge, commit_version, properties,
                              properties_size);
      //        i++;
      //        if (i % 1000 == 0) {
      //        cout << ".";
...
void SnapshotTransaction::aquire_locks_and_insert_vertices() {
  ...
  for (const auto &v : locks_to_aquire) {  // Relies on locks_to_aquire being a
                                           // sorted data structure
    if (v != last_lock) {                  // Dedup locks
      if (!ds->aquire_vertex_lock(v)) {
        if (find(vertices_to_insert.begin(), vertices_to_insert.end(), v) !=
            vertices_to_insert.end()) {
          last_lock_aquired = v;
          if (ds->insert_vertex_version(v, commit_version)) {
            rollbacks.push_back(
                RollbackAction::generate_rollback_insert_vertex(v));
        ...
        last_lock_aquired =
            v;  // TODO needs to be initilized to another value than 0
      }
      last_lock = v;
    }
  }
...
void SnapshotTransaction::release_locks() {
  vertex_id_t last_lock = numeric_limits<vertex_id_t>::max();
  for (auto &v : locks_to_aquire) {  // Relies on locks_to_aquire being a sorted
                                     // data structure
    if (v != last_lock &&
        v <= last_lock_aquired) {  // Dedup locks && do not release locks which
                                   // have not been aquired.
...
void SnapshotTransaction::intersect_neighbourhood_p(vertex_id_t a,
                                                    vertex_id_t b,
                                                    vector<dst_t> &out) {
  // if (write_only) {
  //   throw IllegalOperation("Cannot read with a WriteOnly Transaction");
...
version_t SnapshotTransaction::get_version() const { return read_version; }
...
STL_VersionedTopologyInterface *SnapshotTransaction::raw_ds() { return ds; }
...
void SnapshotTransaction::assert_std_preconditions() {
  ...
  for (auto [e, _, _1] : edges_to_insert) {
    ...
    if (!ds->has_vertex_version(e.src, commit_version) &&
        find(vertices_to_insert.begin(), vertices_to_insert.end(), e.src) ==
            vertices_to_insert.end()) {
      throw VertexDoesNotExistsException(e.src);
    }
    if (!ds->has_vertex_version(e.dst, commit_version) &&
        find(vertices_to_insert.begin(), vertices_to_insert.end(), e.src) ==
            vertices_to_insert.end()) {
...
bool SnapshotTransaction::insert_edge(edge_t edge, char *properties,
...
bool SnapshotTransaction::insert_or_update_edge(edge_t edge, char *properties,
                                                size_t property_size) {
  locks_to_aquire.push_back(edge.src);
