...
#define MIN_VERSION_UPDATER_INTERVAL \
  100  // The interval in which the minimal version is updated, in microseconds.
...
class TransactionManager {
 public:
   ...
   */
  explicit TransactionManager(uint max_threads, bool on);
  ~TransactionManager();
  ...
  SnapshotTransaction getSnapshotTransaction(STL_VersionedTopologyInterface* ti,
                                             bool write_only);
  void getSnapshotTransaction(STL_VersionedTopologyInterface* ti,
                              bool write_only,
                              SnapshotTransaction& existing_transaction_object);

  version_t draw_timestamp(bool commit_timestamp);

  void transactionCompleted(const SLT_Transaction& transaction);

  version_t getMinActiveVersion();
  void update_min_version();

  /**
   * @return a list of all active transaction in descending order. Can contain a
   * sequence of NO_TRANSACTION markers first.
   */
