...
inline version_t inline_version(bool deletion, bool more_versions,
                                version_t version) {
  if (more_versions) {
...
class EdgeBlock {
 public:
  EdgeBlock(dst_t* start, size_t capacity, size_t edges_and_versions,
      ...
      : start(start),
        end(((char*)start) + capacity * sizeof(dst_t) +
            capacity * property_size),
        capacity(capacity),
        edges_and_versions(edges_and_versions),
        properties(properties),
        property_size(property_size) {};

  EdgeBlock(dst_t src, size_t _property_size)
      : start(nullptr),
        end(nullptr),
        capacity(0),
        edges_and_versions(0),
        properties(0),
        property_size(_property_size) {};

  unsigned getSum(version_t version) {
    auto sm = 0;
    auto prop = (weight_t*)properties_start();
    for (auto i = start; i < start + edges_and_versions; i++, prop++) {
      if (is_versioned(*i)) {
        EdgeVersionRecord vr(make_unversioned(*i), i + 1, (char*)prop, true,
                             property_size);
        if (vr.exists_in_version(version)) {
          sm += vr.get_weight(version);
        }
        i++;
      }
  ...
  void build(unsigned _num, dst_t* _edges, char* _properties) {
    capacity = INITIAL_BLOCK_CAPACITY_FOR_TEST;
    edges_and_versions = _num;

    properties = _num;

    start = reinterpret_cast<dst_t*>(
        new char[capacity * sizeof(dst_t) + capacity * property_size]);

    end = reinterpret_cast<char*>(start) + capacity * sizeof(dst_t) +
          capacity * property_size;

    memcpy(start, _edges, edges_and_versions * sizeof(dst_t));
    memcpy(properties_start(), _properties, properties * property_size);
  }
  ...
  bool get_weight(dst_t e, version_t v, char* out) {
    ...
    EdgeVersionRecord vr{make_unversioned(*e_ptr),
                         v_ptr == nullptr ? &dummy_version : v_ptr,
                         properties_start() + property_offset * property_size,
                         true, property_size};
  ...
  bool insert_edge(dst_t e, version_t version, char* properties) {
    ...
    if (pos == start + edges_and_versions ||
        make_unversioned(*pos) != e) {  // No version of this edge exists.
      memmove((char*)(pos + 2), (char*)pos,
              (edges_and_versions - (pos - start)) * sizeof(dst_t));
      ...
      *(pos + 1) = inline_version(false, false, version);
      ...
      this->properties += 1;
    } else if (is_versioned(*pos)) {  // Earlier version of this edge exists and
                                      // pos points to it.
      int offset = pos - start;
      int property_offset = offset - count_versions_before(offset);
      char* property = properties_start() + property_offset * property_size;
      EdgeVersionRecord vr{make_unversioned(*pos), pos + 1, property, true,
                           property_size};
      if (vr.exists_in_version(version)) {
      ...
      memmove((char*)(pos + 2), (char*)(pos + 1),
              (edges_and_versions - ((pos + 1) - start)) * sizeof(dst_t));
      *pos = make_versioned(e);
      *(pos + 1) = inline_version(false, false, FIRST_VERSION);

      auto offset = pos - start;
      offset -= count_versions_before(offset);
      char* property = properties_start() + offset * property_size;
      EdgeVersionRecord vr{make_unversioned(*pos), pos + 1, property, true,
                           property_size};
      vr.write(version, UPDATE, properties);
   ...
   * @param properties
   */
  /******  4b03cb35-7192-4466-bb56-c0e12eb13c66  *******/
  ...
  bool gc(version_t min_version,
          const vector<version_t>& sorted_active_versions) {
    // Removes unncessary versions and shifts remaining destinations and
    // versions forward.
    auto shift =
        0;  // The forward shift to use, increases when versions are removed.
    bool version_remaining = false;
    size_t new_size = edges_and_versions;

    // Tracks how many edges we encountered and left so far, with out counting
    // versions and deleted edges
    auto edges_so_far = 0;
    for (auto i = start; i < start + edges_and_versions; i++) {
      auto e = *i;
      auto v = *(i + 1);
      ...
      if (is_versioned(e)) {
        EdgeVersionRecord vr{make_unversioned(e), (version_t*)i + 1, nullptr,
                             false, property_size};
      ...
      if (is_versioned(e) && !(v & MORE_VERSION_MASK) &&
          timestamp(v) < min_version) {
        if (is_deletion(v)) {
          new_size -= 2;
          shift += 2;
          i += 1;
          ...
          memmove(properties_start() + property_size, properties_start(),
                  edges_so_far * property_size);
          properties -= 1;
        } else {
  ...
  void copy_into(EdgeBlock& other) {
    ...
    memcpy(other.properties_start(), properties_start(),
  ...
  tuple<size_t, size_t> split_into(EdgeBlock& other) {
    ...
    if (is_versioned(start[split - 1])) {  // Keep the versioned edge together
                                           // with its version.
      split -= 1;
    }
    memcpy(other.start, start + split,
           (edges_and_versions - split) * sizeof(dst_t));
    ...
    memcpy((char*)other.end - properties_to_move * property_size,
           (char*)end - properties_to_move * property_size,
           properties_to_move * property_size);
    // Move properties in existing block
    memmove(end - property_split * property_size, properties_start(),
            property_split * property_size);
   ...
   * If b1 and b2 are sorted internally and b1.max < b2.min, then the condition
   * holds afterwards.
   *
   * If b1[elements] is a versioned edge, it moves one element more to keep edge
  ...
  static void move_forward(EdgeBlock& from, EdgeBlock& to, size_t elements) {
    ...
    // #ifdef DEBUG
    //       vector<dst_t> before;
    //       for (auto i = from.start; i < from.start + from.edges_and_versions;
    //       i++) {
    ...
    memmove((char*)(to.start + elements), (char*)to.start,
            to.edges_and_versions * sizeof(dst_t));

    // Move elements from to to from
    memcpy((char*)to.start, (char*)(from.start + elements_not_to_move),
           elements * sizeof(dst_t));

    // Move Properties from to to from
    memcpy(to.properties_start() - properties_to_move * property_size,
           from.properties_start() + properties_not_to_move * property_size,
           properties_to_move * property_size);

    // Move properties in from to the end
    memmove(from.end - properties_not_to_move * property_size,
            from.properties_start(), properties_not_to_move * property_size);
    ...
    //       if (!matching) {
    //         cout << from.edges_and_versions << " " << to.edges_and_versions
    //         << endl; cout << "Moved " << elements << " elements"; cout <<
    //         "Content before" << endl; for (auto i = 0; i < before.size();
    //         i++) {
  ...
  static void move_backward(EdgeBlock& from, EdgeBlock& to, size_t elements) {
    ...
    // Move elements from to to from
    memcpy((char*)(to.start + to.edges_and_versions), (char*)from.start,
           elements * sizeof(dst_t));

    // Move elements in from backwards
    memmove((char*)from.start, (char*)(from.start + elements),
            (from.edges_and_versions - elements) * sizeof(dst_t));

    // Move Properties
    // Make place for properties
    memmove(to.properties_start() - properties_to_move * property_size,
            to.properties_start(), to.properties * property_size);

    // Move properties
    memcpy(to.end - properties_to_move * property_size, from.properties_start(),
           properties_to_move * property_size);
  ...
  size_t count_edges(version_t version) {
    ...
    for (auto i = start; i < start + edges_and_versions; i++) {
      if (!is_versioned(*i)) {
        ...
        EdgeVersionRecord vr(make_unversioned(*i), i + 1, nullptr, false,
                             property_size);
        if (vr.exists_in_version(version)) {
          count++;
        }
  ...
  dst_t* get_single_block_pointer() { return start; }

  size_t get_edges_and_versions() { return edges_and_versions; }

  size_t get_property_count() { return properties; }

  size_t get_block_capacity() { return capacity; }
  ...
  dst_t get_min_edge() { return make_unversioned(*start); }
  ...
  char* properties_start() { return end - properties * property_size; }

  char* properties_end() { return end; }
  ...
  static dst_t* find_upper_bound(dst_t* start, dst_t* end, dst_t value) {
    auto l = 0;
    auto r = end - start;
    while ((r - l) > 16) {  // Incorrect if not ended before r-l > 4 because
                            // there could be an endless loop.
      auto m = l + (r - l) / 2;

      auto v = 0 < m && is_versioned(start[m - 1])
                   ? make_unversioned(start[m - 1])
                   : make_unversioned(start[m]);
      if (value > v) {
        l = m + 1;
      } else if (v == value) {
    ...
    dst_t* ptr = start + l;
    if (ptr != start &&
        is_versioned(*(ptr - 1))) {  // Do not start on a version record.
      ptr -= 1;
    }
    for (; ptr < end; ptr++) {
      auto v = *ptr;
  ...
  void aquire_vertex_lock() { lock.lock(); }

  void aquire_vertex_lock_shared() { lock.lock_shared(); }

  void release_vertex_lock_shared() { lock.unlock_shared(); }

  void release_vertex_lock() { lock.unlock(); }
 ...
 private:
  ...
  size_t size() { return (end - (char*)start); };
  ...
  void insert_properties_by_shift(char* properties, size_t offset) {
    if (offset == 0) {
      ...
      memcpy(properties_start() + (offset - 1) * property_size, properties,
             property_size);
    }
  }
