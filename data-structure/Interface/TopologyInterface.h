#ifndef LIVE_GRAPH_TWO_TOPOLOGYINTERFACE_H
#define LIVE_GRAPH_TWO_TOPOLOGYINTERFACE_H
...
class TopologyInterface {
 public:
  ...
  virtual bool insert_edge(edge_t edge, char* properties,
  ...
  // virtual VersionedBlockedEdgeIterator neighbourhood_blocked_p(vertex_id_t
  // src);
  virtual void* raw_neighbourhood(vertex_id_t src) = 0;
  virtual void intersect_neighbourhood(vertex_id_t a, vertex_id_t b,
                                       vector<dst_t>& out);
  virtual void intersect_neighbourhood_p(vertex_id_t a, vertex_id_t b,
                                         vector<dst_t>& out) = 0;
