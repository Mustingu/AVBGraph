...
class STL_VersionedTopologyInterface {
 public:
  virtual ~STL_VersionedTopologyInterface();
  ...
  /**
   * Will also acquire the vertex lock for the vertex in question, indepdent on
   * if it is added or not.
   *
   * @param v
   * @param version
  ...
  virtual bool get_weight_version_p(edge_t edge, version_t version,
                                    char* out) = 0;

  virtual size_t neighbourhood_size_version(vertex_id_t src, version_t version);
  virtual size_t neighbourhood_size_version_p(vertex_id_t src,
                                              version_t version) = 0;

  virtual bool insert_edge_version(edge_t edge, version_t version) = 0;
  virtual bool insert_edge_version(edge_t edge, version_t version,
                                   char* properties,
   ...
   *
   * @param vertex_lock
   * @return true if the lock has been acquired, false if the logical vertex
   * does not exists and no lock has been aquired.
   */
