...
struct edge_t {
  ...
  static edge_t parse_edge(const string &line, const char seperator, size_t _,
                           bool densify,
                           unordered_map<string, size_t> &densifyer,
    ...
    if (densify) {
      ...
      auto second_seperator_index =
          line.find(seperator, first_seperator_index + 1);

      string src_string = line.substr(0, first_seperator_index);
      string dst_string =
          line.substr(first_seperator_index + 1,
                      second_seperator_index - first_seperator_index - 1);
  ...
  static edge_t parse_edge_densifying(const string &line, const char seperator,
                                      unordered_map<string, size_t> &densifyer,
                                      size_t &next_vertex_id) {
    auto first_seperator_index = line.find(seperator);
    auto second_seperator_index =
  ...
  edge_t opposite() { return {dst, src}; }
...
struct weighted_edge_t {
  ...
  weighted_edge_t(vertex_id_t src, dst_t dst, weight_t weight)
  ...
  static weighted_edge_t parse_edge(const string &line, const char seperator,
                                    size_t weight_position, bool densify,
                                    unordered_map<string, size_t> &densifyer,
                                    size_t &next_vertex_id,
                                    unordered_set<size_t> &vertex_set) {
    edge_t edge = edge_t::parse_edge(line, seperator, weight_position, densify,
                                     densifyer, next_vertex_id, vertex_set);
    ...
    if (weight_position != numeric_limits<size_t>::max()) {
      ...
      t_edge.weight = stod(
          line.substr(weight_index + 1, afterTempIndex - weight_index - 1));
    }
  ...
  weighted_edge_t opposite() { return {dst, src, weight}; }
...
struct temporal_edge_t {
  ...
  temporal_edge_t(vertex_id_t src, dst_t dst, time_t time)
      : src(src), dst(dst), creation_timestamp(time) {};

  static temporal_edge_t parse_edge(const string &line, const char seperator,
                                    size_t temporal_value_position,
                                    bool densify,
                                    unordered_map<string, size_t> &densifyer,
                                    size_t &next_vertex_id,
                                    unordered_set<size_t> &vertex_set) {
    edge_t edge =
        edge_t::parse_edge(line, seperator, temporal_value_position, densify,
                           densifyer, next_vertex_id, vertex_set);
    ...
    if (temporal_value_position != numeric_limits<size_t>::max()) {
      ...
      t_edge.creation_timestamp =
          stoi(line.substr(tempIndex + 1, afterTempIndex - tempIndex - 1));
    }
  ...
  temporal_edge_t opposite() { return {dst, src, creation_timestamp}; }
